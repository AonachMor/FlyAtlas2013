...
{		
	...
	public GeneSearch(String geneQuery, String radioChoice)
	{	
		...
		if(radioChoice.equals("geneSymbol"))
		{
			parQ = DBQuery.getParamQuery("GENE_BY_SYMBOL");
		}
		else if(radioChoice.equals("geneName"))
		{
			parQ = DBQuery.getParamQuery("GENE_BY_NAME");
		}
		else if(radioChoice.equals("CGnum"))
		{
			parQ = DBQuery.getParamQuery("GENE_BY_CGNUM");
		}
		else if(radioChoice.equals("FBgene"))
		{
			parQ = DBQuery.getParamQuery("GENE_BY_FBGN");
		}
		else
		{
			System.out.println("Error in radio choice at GeneSearch");
		}
		...
		try 
		{
			...
			prepStat.setString(1, geneQuery);	
			if(radioChoice.equals("geneSymbol") || radioChoice.equals("geneName"))
			{
				prepStat.setString(2, geneQuery);		// for Roman version
			}
			...
			if(resSet.first())
			{
				...
				{	
					// checks if new set by by initial value of atStart or change in values of FBgn or Probe Set ID
